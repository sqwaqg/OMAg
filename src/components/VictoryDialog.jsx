...
function VictoryDialog({ onComplete, score, type, playSfx }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 'clamp(5%, 2vw, 8%)', width: 'clamp(200px, 32vw, 320px)', animation: 'slideInLeft 0.5s ease' }}>
      ...
      {showTablet && (
        <div style={{
          position: 'absolute',
          bottom: 'clamp(8%, 5vh, 12%)',
          left: 'clamp(12%, 5vw, 18%)',
          width: isHappy ? 'clamp(120px, 20vw, 180px)' : 'clamp(140px, 22vw, 200px)',
      ...
      <div style={{
        position: 'absolute', bottom: 'clamp(25%, 15vh, 40%)', left: '50%', transform: 'translateX(-50%)',
        width: 'clamp(280px, 65vw, 600px)', backgroundColor: 'rgba(255,255,255,0.96)',
        borderRadius: 'clamp(30px, 8vw, 48px)', padding: 'clamp(20px, 4vw, 35px) clamp(25px, 6vw, 40px)',
        ...
        <h2 style={{ color: '#2e7d32', marginBottom: 'clamp(15px, 3vw, 20px)', fontSize: 'clamp(1.5rem, 5vw, 2rem)', fontWeight: 'bold' }}>Поздравляем!</h2>
        <p style={{ fontSize: 'clamp(1rem, 3vw, 1.3rem)', lineHeight: '1.5', color: '#333', marginBottom: 'clamp(20px, 4vw, 30px)' }}>
